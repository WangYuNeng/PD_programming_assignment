...
DijstraMap::DijstraMap ()
{
    ...
    vCapacity = new short* [_htiles];
    hCapacity = new short* [_htiles];
    vLoad = new short* [_htiles];
    hLoad = new short* [_htiles];
    ...
    for (int i = 0; i < _htiles; i++)
    {
        ...
        vCapacity[i] = new short [_vtiles];
        hCapacity[i] = new short [_vtiles];
        vLoad[i] = new short [_vtiles];
        hLoad[i] = new short [_vtiles];
...
bool Router::preRoute ()
{
    ...
    for (multimap<int, Net*>::iterator it = orderedNets.begin(); it != orderedNets.end(); ++it)
    {
        ...
        dMap->refreshConnection();
        _orderedSubnet.clear();
        if ( time(NULL) - startTime > preRouteStopTime )
        {
            return 1;
        }
